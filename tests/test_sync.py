...
def test_merge_url_as_packages(from_line):
...
def test_sync_with_pip_flags(check_call, from_line):
    sync({from_line('django==1.8')}, set(), pip_flags=['--flag1', '--flag2'])
    for call in check_call.call_args_list:
        assert '--flag1' in call[0][0]
        assert '--flag2' in call[0][0]
...
def test_sync_verbose(check_call, from_line):
    ...
    for call in check_call.call_args_list:
        assert '-q' not in call[0][0]


@mock.patch('piptools.sync.check_call')
def test_sync_virtual_env(check_call, from_line, monkeypatch):
    monkeypatch.setenv('VIRTUAL_ENV', '/bin/bash')

    sync({from_line('django==1.8')}, set())

    for call in check_call.call_args_list:
        assert 'pip' == call[0][0][0]


@mock.patch.object(sys, 'argv', ['/usr/local/bin/pip-sync'])
@mock.patch('piptools.sync.check_call')
def test_sync_no_virtual_env(check_call, from_line, monkeypatch):
    """pip should be in the same directory as pip-sync entry-point script"""
    monkeypatch.setenv('VIRTUAL_ENV', '')

    sync({from_line('django==1.8')}, set())
    for call in check_call.call_args_list:
        assert '/usr/local/bin/pip' == call[0][0][0]
