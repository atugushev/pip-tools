...
def test_command_line_setuptools_output_file(pip_conf):
    ...
    with runner.isolated_filesystem():
        ...
        """))
        package.close()
        out = runner.invoke(cli, ['--output-file', 'output.txt'])

        assert '#    pip-compile --output-file output.txt setup.py' in out.output
