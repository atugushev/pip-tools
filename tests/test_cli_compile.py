...
@pytest.mark.parametrize('options, expected_output_file', [
    # Without any options default output file should be "requirements.txt"
    ([], 'requirements.txt'),
    # With the option --output-file=output.txt output file should be "output.txt"
...
def test_cert_option(MockPyPIRepository, runner, option, attr, expected):
    """
    The options --cert and --client-crt options has to be passed to the PyPIRepository.
    """
    with open('requirements.in', 'w') as req_in:
        req_in.write('six==1.10.0')
